...
State detection now resolves the exact `login.fulfilled` property access passed
to `builder.addCase(...)` when that canonical lifecycle Event already exists in
the graph. The auth acceptance is therefore GREEN:

```text
login.fulfilled --UPDATES--> auth
```

Unknown property accesses remain omitted rather than becoming dangling edges.
...
## Listener middleware boundary

The corpus registers listener middleware through
`apiSlice.endpoints.addNewPost.matchFulfilled`. That trigger is generated by
RTK Query rather than declared as an explicit Redux Event in the scanned
source. A `LISTENS_TO` relation would therefore require inventing an Event and
is intentionally absent from this Redux-only validation.

The three independent projections retained for broader validation are the
notification Event-to-State relation, the isolated handwritten thunk Handler,
and the typed async thunk lifecycle topology completed by its auth State
update.
