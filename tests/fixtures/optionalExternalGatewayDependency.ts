...
function getGateway(source: Gateways | undefined, kind: Command["kind"]) {
  switch (kind) {
    case "Like.Add":
      return source?.likes;
    case "Comment.Create":
...
async function sendCommand({ gateway }: { gateway: object | undefined }): Promise<void> {
  if (!gateway) return;

  if (hasAdd(gateway)) await gateway.add();
  if (hasCreate(gateway)) await gateway.create();
}
...
const handlerFactory = () => {
  ...
  listen({
    ...
    effect: async () => {
      ...
      await sendCommand({ gateway: gateway });
    },
  });
