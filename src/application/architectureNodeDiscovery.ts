...
import type { ArchitectureNodeDiscoveryAlias } from "./architectureScanner.js";
...
const matchRank = (
  ...
  query: string,
  aliases: readonly ArchitectureNodeDiscoveryAlias[],
): number | undefined => {
  ...
  const nodeAliases = aliases.find((candidate) => candidate.nodeId === node.id)?.aliases ?? [];
  if (nodeAliases.some((alias) => normalize(alias) === query)) return 6;
  if (nodeAliases.some((alias) => normalize(alias).includes(query))) return 7;
  return undefined;
};
...
export const findArchitectureNodes = (
  ...
  const matches = graph.nodes
    .map((node) => ({ node, rank: matchRank(node, normalizedQuery, aliases) }))
    .filter(
